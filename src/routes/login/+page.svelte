<script>
  ...
  <div class="bg- h-screen w-max[1440px] flex flex-col md:flex-row gap-5 md:gap-10 justify-center md:justify-evenly items-center w-full pt-10 md:pt-0">
    <div class="h-screen w-screen max-w-[1200px] flex justify-center items-center overflow-hidden text-">
      <div class="card bg-dark flex justify-center items-center w-80 shadow-xl p-5">
        ...
        <form class="card-body" on:submit={log_in}>
          ...
          <!-- Add loading indicator inside the login button -->
          <input type="submit" value="Log in" class="btn btn-neutral rounded-full bg-zinc-950 shadow-xl relative border shadow-xl shadow-indigo-600/40" />
          {#if loading}
