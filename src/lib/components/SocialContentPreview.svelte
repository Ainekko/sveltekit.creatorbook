	...
	<div class="grid md:grid-cols-3 gap-5">
      ...
      {/if} -->
		...
		{#if linkedinPosts.length > 0}
			<div
				class="relative bg-zinc-950 rounded-lg border border-zinc-800 overflow-hidden group hover:border-purple-500/50 transition-all duration-200"
			>
				<div class="absolute top-3 right-3 z-10">
				...
				<div class="p-4">
					<div class="flex items-center mb-3">
						<div
							class="w-10 h-10 rounded-full bg-purple-900/30 flex items-center justify-center text-lg"
						>
							in
						</div>
						<div class="ml-3">
		...
		{#if redditPosts.length > 0}
			<div
				class="relative bg-zinc-950 rounded-lg border border-zinc-800 overflow-hidden group hover:border-orange-500/50 transition-all duration-200"
			>
				<div class="absolute top-3 right-3 z-10">
				...
				<div class="p-4">
					<div class="flex items-center mb-3">
						<div
							class="w-10 h-10 rounded-full bg-orange-900/30 flex items-center justify-center text-lg"
						>
					...
					<p class="text-sm text-zinc-300 mb-4 line-clamp-3">
						{redditPosts[0]?.content ||
							redditPosts[0]?.content_summary ||
							'Reddit post content not available'}
					</p>
