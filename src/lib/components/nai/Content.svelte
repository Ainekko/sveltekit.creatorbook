...
<div class="space-y-6">
  ...
  <div class="flex justify-between items-center">
    ...
    <div class="flex items-center gap-3">
      ...
      <button 
        ...
        disabled={isLoading}
        class="inline-flex items-center gap-2 px-4 py-2 rounded-lg bg-zinc-900 hover:bg-zinc-700 text-white text-sm font-medium transition-all disabled:opacity-50 disabled:cursor-not-allowed"
      >
  ...
  {#if filteredPosts.length === 0}
    <div class="bg-white rounded-xl border border-zinc-200 p-12 text-center">
      ...
      <h3 class="text-lg font-semibold text-zinc-900 mb-2">No Posts Yet</h3>
      <p class="text-zinc-500">Generate blog posts to get started</p>
    </div>
  {:else}
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
      {#each filteredPosts as post (post.id)}
        <button
          ...
          class="text-left bg-white rounded-xl border border-zinc-200 p-6 hover:border-zinc-300 hover:shadow-md transition-all duration-200 group"
        >
  ...
  <div class="fixed inset-0 bg-black/50 backdrop-blur-sm z-40" on:click={closeDrawer}>
    ...
    >
      ...
      <div class="flex items-start justify-between p-6 border-b border-zinc-200 bg-zinc-50">
        <div class="flex-1">
          <span class={`inline-flex items-center px-2.5 py-1 rounded-md text-xs font-medium mb-3 ${getStatusColor(selectedPost.status)}`}>
            {selectedPost.status}
          </span>
      ...
      <div class="border-t border-zinc-200 p-6 bg-zinc-50 flex items-center gap-3">
        {#if isEditing}
          <button
            ...
            disabled={isSaving}
            class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg bg-zinc-900 hover:bg-zinc-700 text-white text-sm font-medium transition-all disabled:opacity-50 active:scale-95"
          >
            <Save class="w-4 h-4" />
          ...
          {#if selectedPost.status === 'draft'}
            <div class="relative">
              <button
                ...
                class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-white text-sm font-medium transition-all active:scale-95"
              >
                Publish
              ...
              {#if showPublishMenu}
                <div class="absolute bottom-full mb-2 left-0 w-56 bg-white rounded-lg shadow-xl border border-zinc-200 py-1 z-10">
                  <button
