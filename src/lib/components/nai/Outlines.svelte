				...
				<div class="flex items-center gap-3 mb-2">
					...
					<div>
						...
						<p class="text-sm text-white/80">
							Creating post{selectedOutlineIds.length !== 1 ? 's' : ''}
						</p>
					</div>
				</div>
			</div>
...
<div class="space-y-6 w-full">
	...
	{#if outlines.length === 0 && !isLoading}
		...
		>
			...
			<h3 class="text-xl font-bold text-zinc-900 mb-2">No Outlines Yet</h3>
		...
		>
			...
			<div class="p-4 space-y-4">
				{#each outlines as outline, index (outline.id)}
					...
					<div
						class="gradient-border overflow-hidden transition-all duration-300 hover:shadow-lg cursor-pointer bg-white rounded-2xl border border-zinc-200"
						...
						<div class="p-4 flex items-center gap-4 bg-white">
								...
								<div class="flex items-center gap-4 text-xs text-gray-500">
									...
									{#if outline.target_keywords && outline.target_keywords.length > 0}
										<div class="flex items-center gap-1">
											...
											{outline.target_keywords.length} keyword{outline.target_keywords.length !== 1
												? 's'
												: ''}
										</div>
									{/if}
							...
							<div class="flex items-center gap-2">
								<button
									...
									class="bg-zinc-900 hover:bg-zinc-800 text-white px-4 py-2 rounded-lg text-sm font-medium transition-colors disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2 shadow-md shadow-zinc-900/10"
								>
									<ArrowRight class="w-4 h-4" />
									Generate Post
								</button>
								<div
									class="w-8 h-8 rounded-lg bg-gradient-to-br {gradient} flex items-center justify-center"
								>
								...
								<div class="mb-4">
									...
									<div class="flex flex-wrap gap-2">
										{#each outline.target_keywords as keyword}
											<span
												class="inline-flex items-center px-2.5 py-1 rounded-lg text-xs font-medium bg-zinc-100 text-zinc-700 border border-zinc-200"
											>
												{keyword}
											</span>
										{/each}
										...
										<div class="space-y-2 text-sm text-gray-700">
											{#each parseOutlineContent(outline.outline_content) as line}
												<div class="flex items-start gap-2">
													<div
														class="w-1.5 h-1.5 rounded-full bg-gradient-to-br {gradient} mt-2 flex-shrink-0"
													></div>
