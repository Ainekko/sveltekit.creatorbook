<script>
	...
	let platforms = [
		...
		// { name: 'Twitter', enabled: false, icon: 'twitter', color: 'text-blue-400' }, // Hidden
...
<div class="w-full max-w-3xl mx-auto rounded-3xl border border-zinc-200 bg-white overflow-hidden">
		...
		<div class="mt-6 flex items-center w-full">
			{#each Array(totalSteps) as _, i}
				<div class="flex-1 relative">
					...
					{#if i < totalSteps - 1}
						<div
							class="absolute top-0 right-0 w-4 h-2 bg-gradient-to-r {i < currentStep - 1
								? 'from-white to-white'
								: 'from-white/30 to-white/30'}"
						></div>
					{/if}
				</div>
			{/each}
	...
	<div class="p-6">
			...
			<div class="grid grid-cols-2 gap-4">
				{#each platforms as platform}
					...
					>
							...
							<div class="p-2 bg-zinc-100 rounded-full mr-3">
								{#if platform.icon === 'reddit'}
									...
									>
										...
										/>
										<path
											d="M15 10.5C15 12.433 13.433 14 11.5 14S8 12.433 8 10.5 9.567 7 11.5 7 15 8.567 15 10.5z"
											fill="white"
										/>
									</svg>
								{:else if platform.icon === 'linkedin'}
									...
									>
										<path
											d="M19 3H5a2 2 0 00-2 2v14a2 2 0 002 2h14a2 2 0 002-2V5a2 2 0 00-2-2zM9 17H6.5v-7H9v7zm-1.25-8a1.25 1.25 0 110-2.5 1.25 1.25 0 010 2.5zM18 17h-2.5v-4c0-1.5-1-2-1.5-2s-1.5.5-1.5 2v4h-2.5v-7h2.5v1.5c.5-.5 1.5-1.5 2.5-1.5s2.5 1 2.5 3v4z"
										/>
									</svg>
								...
								{:else}
									...
									>
										<path
											d="M12 2C6.486 2 2 6.486 2 12s4.486 10 10 10 10-4.486 10-10S17.514 2 12 2zm0 19.5c-5.244 0-9.5-4.256-9.5-9.5S6.756 2.5 12 2.5s9.5 4.256 9.5 9.5-4.256 9.5-9.5 9.5z"
										/>
						...
						<div class="relative">
							<div
								class="w-10 h-6 rounded-full {platform.enabled
									? 'bg-gradient-to-r from-purple-500 to-pink-500'
									: 'bg-zinc-200'} transition-all duration-300"
							></div>
							<div
								class="absolute top-1 {platform.enabled
									? 'left-5'
									: 'left-1'} w-4 h-4 bg-white rounded-full transition-all duration-300"
							></div>
						</div>
					</button>
				{/each}
		...
		{:else if currentStep === 2}
			...
			<p class="text-sm text-zinc-500 mb-6">
				Provide the necessary API keys for each selected platform
			</p>
						...
						<div class="p-5 rounded-xl border border-zinc-200">
							...
							{#if platform.name === 'Reddit'}
								<div class="space-y-3">
									<div>
										<label
											for="reddit-clientID"
											class="block text-sm font-medium text-zinc-700 mb-1">Client ID</label
										>
										<input
											id="reddit-clientID"
											bind:value={apiCredentials.reddit.clientId}
											type="text"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
											placeholder="Enter your Reddit Client ID"
										/>
									</div>
									<div>
										<label
											for="reddit-clientSecret"
											class="block text-sm font-medium text-zinc-700 mb-1">Client Secret</label
										>
										<input
											id="reddit-clientSecret"
											bind:value={apiCredentials.reddit.clientSecret}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="reddit-refreshToken"
											class="block text-sm font-medium text-zinc-700 mb-1">Refresh Token</label
										>
										<input
											id="reddit-refreshToken"
											bind:value={apiCredentials.reddit.refreshToken}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
											placeholder="Enter your Reddit Refresh Token"
										/>
									</div>
								</div>
							{:else if platform.name === 'LinkedIn'}
								<div class="space-y-3">
									<div>
										<label
											for="linkedin-clientID"
											class="block text-sm font-medium text-zinc-700 mb-1">Client ID</label
										>
										<input
											id="linkedin-clientID"
											bind:value={apiCredentials.linkedin.clientId}
											type="text"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="linkedin-clientSecret"
											class="block text-sm font-medium text-zinc-700 mb-1">Client Secret</label
										>
										<input
											id="linkedin-clientSecret"
											bind:value={apiCredentials.linkedin.clientSecret}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="linkedin-accessToken"
											class="block text-sm font-medium text-zinc-700 mb-1">Access Token</label
										>
										<input
											id="linkedin-accessToken"
											bind:value={apiCredentials.linkedin.accessToken}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
							...
							{:else if platform.name === 'Twitter'}
								<div class="space-y-3">
									<div>
										<label for="twitter-apiKey" class="block text-sm font-medium text-zinc-700 mb-1"
											>API Key</label
										>
										<input
											id="twitter-apiKey"
											bind:value={apiCredentials.twitter.apiKey}
											type="text"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="twitter-apiSecret"
											class="block text-sm font-medium text-zinc-700 mb-1">API Secret</label
										>
										<input
											id="twitter-apiSecret"
											bind:value={apiCredentials.twitter.apiSecret}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="twitter-accessToken"
											class="block text-sm font-medium text-zinc-700 mb-1">Access Token</label
										>
										<input
											id="twitter-accessToken"
											bind:value={apiCredentials.twitter.accessToken}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
							...
							{:else}
								<div class="space-y-3">
									<div>
										<label for="wordpress-url" class="block text-sm font-medium text-zinc-700 mb-1"
											>WordPress URL</label
										>
										<input
											id="wordpress-url"
											bind:value={apiCredentials.wordpress.url}
											type="text"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="wordpress-username"
											class="block text-sm font-medium text-zinc-700 mb-1">Username</label
										>
										<input
											id="wordpress-username"
											bind:value={apiCredentials.wordpress.username}
											type="text"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
									...
									<div>
										<label
											for="wordpress-password"
											class="block text-sm font-medium text-zinc-700 mb-1"
											>Password or API Key</label
										>
										<input
											id="wordpress-password"
											bind:value={apiCredentials.wordpress.password}
											type="password"
											class="w-full px-3 py-2 border border-zinc-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500"
											placeholder="Enter your WordPress password or API key"
										/>
									</div>
								</div>
							{/if}
		...
		{:else if currentStep === 3}
			...
			<p class="text-sm text-zinc-500 mb-6">
				Configure how the AI agent should handle your content
			</p>
					...
					<div class="flex items-center justify-between">
						<div>
							...
							<p class="text-sm text-zinc-500">
								AI will send content for your review before publishing
							</p>
						</div>
						<div class="relative">
							<div
								class="w-10 h-6 rounded-full bg-gradient-to-r from-purple-500 to-pink-500 transition-all duration-300"
							></div>
							<div
								class="absolute top-1 left-5 w-4 h-4 bg-white rounded-full transition-all duration-300"
							></div>
						</div>
					</div>
				</div>
		...
		{:else if currentStep === 4}
			...
			<div class="flex flex-col items-center justify-center py-6">
				...
				<p class="text-zinc-500 text-center mb-6">
					Your AI agent is now configured and ready to grow your online presence
				</p>

				<div class="text-center p-4 bg-zinc-50 rounded-xl w-full">
					<h4 class="font-medium text-zinc-900 mb-2">Next steps</h4>
					<p class="text-sm text-zinc-500">
						Your first content suggestions will be ready within 24 hours
					</p>
				</div>

				<button
					class="mt-6 inline-flex items-center justify-center rounded-full bg-gradient-to-r from-purple-500 to-pink-500 px-6 py-3 font-medium text-white hover:from-purple-600 hover:to-pink-600 transition-all duration-200"
				>
					Go to dashboard
				</button>
			</div>
	...
	<div class="p-6 border-t border-zinc-100 flex justify-between">
		{#if currentStep > 1 && currentStep < 4}
			<button
				on:click={prevStep}
				class="px-5 py-2 border border-zinc-300 rounded-lg font-medium text-zinc-700 hover:bg-zinc-50 transition-all duration-200"
			>
				Back
			</button>
		{:else}
		...
		{#if currentStep < 3}
			<button
				on:click={nextStep}
				class="px-5 py-2 rounded-lg font-medium text-white bg-gradient-to-r from-purple-500 to-pink-500 hover:from-purple-600 hover:to-pink-600 transition-all duration-200"
			>
				Continue
			</button>
		{:else if currentStep === 3}
