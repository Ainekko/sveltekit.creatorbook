...
<div class="mb-8 relative z-10 w-4/5 max-w-[500px] mx-auto gap-2 flex flex-col justify-center items-center ">
  <div class="bg-zinc-800/10 text-black backdrop-blur-sm rounded-xl shadow-lg shadow-black/20 border p-1 flex flex-col w-full justify-center items-center md:flex-row overflow-x-auto no-scrollbar">
    {#each mappedNavItems as item}
      <a 
        href={item.href} 
        class="w-full md:w-auto px-6 py-3 rounded-lg text-sm font-medium flex items-center gap-2 min-w-max transition-all duration-200 {isActive(item.href) ? 'bg-zinc-800 text-zinc-200' : 'text-zinc-800 hover:bg-zinc-800 hover:text-zinc-200'}"
      >
        {@html item.icon}
        {item.title}
      </a>
