...
<button
	...
	disabled={isLoading}
	class="flex w-full max-w-sm items-center justify-center gap-3 rounded-xl bg-zinc-200 {size === 'large' ? 'px-4 py-2 font-medium' : 'px-2 py-1.5 font- '} text-zinc-950 transition-colors hover:bg-zinc-300 disabled:cursor-not-allowed disabled:opacity-70"
>
	{#if isLoading}
		<div class="h-{size === 'large' ? '5' : '4'} w-{size === 'large' ? '5' : '4'} animate-spin rounded-full border-2 border-zinc-950 border-t-transparent" />
		<span>Redirecting...</span>
	{:else}
		<svg xmlns="http://www.w3.org/2000/svg" class="h-{size === 'large' ? '5' : '4'} w-{size === 'large' ? '5' : '4'}" viewBox="0 0 48 48">
