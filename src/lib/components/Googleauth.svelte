<script lang="ts">
    ...
    function initializeGoogle() {
        ...
        google.accounts.id.initialize({
            ...
            context: 'signin' // Explicitly set context for better compatibility
        });
    ...
    function initiateLogin() {
        ...
        try {
            // Using prompt() with error handling
            google.accounts.id.prompt((notification) => {
                if (notification.isNotDisplayed() || notification.isSkippedMoment()) {
                    // If prompt is not displayed or skipped, fall back to manual redirect
                    console.warn('Google prompt was not displayed or was skipped. Reason:', notification.getNotDisplayedReason() || notification.getSkippedReason());
                    isLoading = false;
                    
                    // Fallback to Google OAuth redirect flow
                    initiateRedirectFlow();
                }
                
                if (notification.isDismissedMoment()) {
                    console.info('Google prompt was dismissed. Reason:', notification.getDismissedReason());
                    isLoading = false;
                }
            });
        } catch (error) {
            ...
            // Fallback to manual redirect flow if prompt fails
            initiateRedirectFlow();
        }
    }
    
    // Fallback OAuth redirect flow
    function initiateRedirectFlow() {
        const client_id = '144652246407-itelihabs7ns4brsead66aion8o5uo9d.apps.googleusercontent.com';
