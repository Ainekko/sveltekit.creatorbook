...
>
	<div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
		...
		<!-- Comparison Table -->
		<div
			class="relative overflow-x-auto rounded-[2rem]"
			style="opacity: {visible ? 1 : 0}; transform: translateY({visible
				? 0
		...
		>
			<!-- Outer card -->
			<div
				class="min-w-[700px] bg-[#0F1114] border border-white/[0.07] rounded-[2rem] overflow-hidden shadow-2xl"
			>
				...
				<div class="grid grid-cols-4 border-b border-white/[0.07]">
					...
					{#each cols as col, i}
						<div
							class="p-5 md:p-7 border-l border-white/[0.07] {col.highlight
								? 'relative'
								: ''}"
						>
							{#if col.highlight}
								...
								></div>
								<div
									class="absolute inset-0 bg-violet-500/[0.04] pointer-events-none"
								></div>
							{/if}
							<div class="relative z-10">
								<p
									class="font-semibold text-sm md:text-base {col.highlight
										? 'text-white'
										: 'text-zinc-400'} mb-1"
								>
									{col.label}
				...
				{#each rows as row, ri}
					...
					>
						...
						<div class="px-6 md:px-8 py-5 flex items-center">
							<span class="text-sm md:text-[15px] text-zinc-400 leading-snug"
								>{row.label}</span
							>
						</div>
						...
						<!-- In-house -->
						<div
							class="px-5 md:px-7 py-5 flex items-center justify-center border-l border-white/[0.04]"
						>
							{#if icon(row.inhouse) === 'yes'}
								<div
									class="w-7 h-7 rounded-full bg-zinc-700/50 border border-zinc-600/50 flex items-center justify-center"
								>
									<Check class="w-3.5 h-3.5 text-zinc-300" />
								</div>
							{:else if icon(row.inhouse) === 'no'}
								<div
									class="w-7 h-7 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center"
								>
									<X class="w-3.5 h-3.5 text-red-400" />
								</div>
							{:else}
								<div
									class="w-7 h-7 rounded-full bg-amber-500/10 border border-amber-500/20 flex items-center justify-center"
								>
						...
						<div
							class="px-5 md:px-7 py-5 flex items-center justify-center border-l border-white/[0.04]"
						>
							{#if icon(row.saas) === 'yes'}
								<div
									class="w-7 h-7 rounded-full bg-zinc-700/50 border border-zinc-600/50 flex items-center justify-center"
								>
									<Check class="w-3.5 h-3.5 text-zinc-300" />
								</div>
							{:else if icon(row.saas) === 'no'}
								<div
									class="w-7 h-7 rounded-full bg-red-500/10 border border-red-500/20 flex items-center justify-center"
								>
									<X class="w-3.5 h-3.5 text-red-400" />
						...
						<!-- Flowjoy — highlighted -->
						<div
							class="px-5 md:px-7 py-5 flex items-center justify-center border-l border-white/[0.04] relative"
						>
							<div
								class="absolute inset-0 bg-violet-500/[0.04] pointer-events-none"
							></div>
							{#if icon(row.flowjoy) === 'yes'}
								<div
									class="relative w-7 h-7 rounded-full bg-violet-500/20 border border-violet-400/40 flex items-center justify-center shadow-[0_0_12px_rgba(139,92,246,0.3)]"
								>
									<Check class="w-3.5 h-3.5 text-violet-300" />
								</div>
							{:else if icon(row.flowjoy) === 'no'}
								<div
		...
		<!-- Legend -->
		<div
			class="flex items-center justify-center gap-6 mt-8 text-xs text-zinc-600"
