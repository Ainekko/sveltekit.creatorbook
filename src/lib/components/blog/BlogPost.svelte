...
<article class="blog-post">
	{#if showMetadata}
		...
		<div
			class="hero-section relative overflow-hidden bg-gradient-to-br {gradientClass} rounded-3xl mb-12"
		>
			<div class="absolute inset-0 bg-black/20"></div>
			<div
				class="absolute top-0 right-0 w-64 h-64 bg-white/10 rounded-full blur-3xl gentle-float"
			></div>
			<div
				class="absolute bottom-0 left-0 w-96 h-96 bg-white/5 rounded-full blur-3xl gentle-float"
			...
			<header class="post-header relative z-10 px-8 py-16 md:px-12 md:py-20">
				<div class="post-meta-top flex items-center gap-3 mb-6">
					<span
						class="content-type bg-white/20 backdrop-blur-sm text-white px-4 py-1.5 rounded-full text-xs uppercase font-semibold tracking-wider border border-white/30"
					>
						{post.content_type || 'Article'}
					</span>
					{#if post.primary_keyword}
						<span
							class="primary-keyword bg-white text-zinc-900 px-4 py-1.5 rounded-full text-xs font-semibold"
						>
				...
				<h1
					class="post-title text-4xl md:text-5xl lg:text-6xl font-bold text-white mb-6 leading-tight drop-shadow-lg"
				>
					{post.title}
				</h1>
				...
				{#if post.meta_description}
					<p
						class="post-description text-xl md:text-2xl text-white/90 mb-8 max-w-3xl leading-relaxed font-light"
					>
		...
		{#if post.secondary_keywords?.length > 0}
			<footer class="post-footer mt-16 pt-8 border-t border-zinc-200">
				<div class="flex items-center gap-2 mb-4">
					<div
						class="w-8 h-8 bg-gradient-to-br from-blue-500 to-purple-600 rounded-lg flex items-center justify-center gentle-float"
					>
...
<style>
	...
	/* ===== ENHANCED TABLE STYLES - WORKING VERSION ===== */
	:global(.prose table) {
		width: 100% !important;
	...
	:global(.prose tbody tr:last-child td:first-child) {
		border-bottom-left-radius: 0.75rem;
	}

	:global(.prose tbody tr:last-child td:last-child) {
	...
	:global(.prose tbody tr:nth-child(even):hover) {
		background: linear-gradient(to right, #f4f4f5, #f9fafb);
	}

	/* First Column Emphasis (metric names) */
	...
	:global(.prose tbody tr:hover td:first-child) {
		color: #18181b !important;
	}

	/* Left align all columns for readability */
	:global(.prose td:not(:first-child)) {
		text-align: left !important;
		font-variant-numeric: tabular-nums;
	}

	:global(.prose th:not(:first-child)) {
		text-align: left !important;
	}
